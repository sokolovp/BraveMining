...
bool BraveComponentInstallerPolicy::VerifyInstallation(
   ...
   LOG(INFO) << " BraveComponentInstallerPolicy::VerifyInstallation " << install_dir.value();
...
base::FilePath BraveComponentInstallerPolicy::GetRelativeInstallDir() const {
  ...
  base::FilePath fp = base::FilePath(
      // Convert to wstring or string depending on OS
      base::FilePath::StringType(extension_id.begin(), extension_id.end()));
      
   LOG(INFO) << " BraveComponentInstallerPolicy::GetRelativeInstallDir " << extension_id << " : " << fp.value();
   return fp;
}
...
std::string BraveComponentInstallerPolicy::GetName() const {
   LOG(INFO) << "BraveComponentInstallerPolicy::GetName "  << name_;
  return name_;
}
...
void RegisterComponent(
  ...
  LOG(INFO) << "BraveComponentInstallerPolicy::RegisterComponent "  << name << base64_public_key;
}
