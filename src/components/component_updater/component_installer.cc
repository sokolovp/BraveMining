...
Result ComponentInstaller::InstallHelper(
  ...
  LOG(INFO) << "ComponentInstaller::InstallHelper 1"; 
  auto local_manifest = update_client::ReadManifest(unpack_path);
  if (!local_manifest) {
      LOG(INFO) << "ComponentInstaller::InstallHelper bad manifest"; 
...
LOG(INFO) << "ComponentInstaller::InstallHelper 2"; 
  if (!manifest_version.IsValid()) {
      LOG(INFO) << "ComponentInstaller::InstallHelper INVALID_VERSION";
    return Result(InstallError::INVALID_VERSION);
  }
  if (current_version_.CompareTo(manifest_version) > 0) {
      LOG(INFO) << "ComponentInstaller::InstallHelper VERSION_NOT_UPGRADED";
  ...
  if (!PathService::Get(DIR_COMPONENT_USER, &local_install_path)) {
      LOG(INFO) << "ComponentInstaller::InstallHelper NO_DIR_COMPONENT_USER";
  ...
  if (base::PathExists(local_install_path)) {
    if (!base::DeleteFile(local_install_path, true)) {
        LOG(INFO) << "ComponentInstaller::InstallHelper CLEAN_INSTALL_DIR_FAILED";
      return Result(InstallError::CLEAN_INSTALL_DIR_FAILED);
    }
  }
LOG(INFO) << "ComponentInstaller::InstallHelper 3"; 
  ...
  if (!base::Move(unpack_path, local_install_path)) {
    ...
    LOG(INFO) << "ComponentInstaller::InstallHelper MOVE_FILES_ERROR";
  ...
  if (!base::SetPosixFilePermissions(local_install_path, 0755)) {
    ...
    LOG(INFO) << "ComponentInstaller::InstallHelper SET_PERMISSIONS_FAILED";            
    return Result(InstallError::SET_PERMISSIONS_FAILED);
  }
#endif  // defined(OS_CHROMEOS)
LOG(INFO) << "ComponentInstaller::InstallHelper 4"; 
  ...
  if (result.error) {
      LOG(INFO) << "ComponentInstaller::InstallHelper OnCustomInstall " << result.error;            
    return result;
  }
LOG(INFO) << "ComponentInstaller::InstallHelper 5"; 
  if (!installer_policy_->VerifyInstallation(*local_manifest,
                                             local_install_path)) {
     LOG(INFO) << "ComponentInstaller::InstallHelper INSTALL_VERIFICATION_FAILED " << local_install_path.value();  
...
LOG(INFO) << "ComponentInstaller::InstallHelper 6"; 
...
void ComponentInstaller::FinishRegistration(
  ...
  LOG(INFO) << "TEST" << installer_policy_->GetName();
